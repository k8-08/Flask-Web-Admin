...
import {useDictApi} from '/@/api/v1/system/dict';
...
const getList = () => {
  ...
  useDictApi().getLookupList(state.listQuery)
      .then(res => {
...
const saveOrUpdateLookup = () => {
  lookupFormRef.value.validate((valid: any) => {
    if (valid) {
      useDictApi().saveOrUpdateLookup(state.lookupForm)
          .then(() => {
...
const deleted = (row: any) => {
  ElMessageBox.confirm('此操作将永久删除, 是否继续?', '提示', {
    ...
    type: 'warning',
      ...
      .then(() => {
        useDictApi().delLookup({id: row.id})
            .then(() => {
...
const getLookupValueList = () => {
  ...
  useDictApi().getLookupValue(state.lookupValueQuery)
      .then(res => {
...
const saveOrUpdateLookupValue = (row: any) => {
  useDictApi().saveOrUpdateLookupValue(row)
      .then((res: any) => {
...
const deletedLookupValue = (row: any) => {
  ElMessageBox.confirm('此操作将永久删除, 是否继续?', '提示', {
    ...
    type: 'warning',
      ...
      .then(() => {
        useDictApi().delLookupValue({id: row.id})
            .then(() => {
              ElMessage.success('操作成功！');
              getLookupValueList()
            })
